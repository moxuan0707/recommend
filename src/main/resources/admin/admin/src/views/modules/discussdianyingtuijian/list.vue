		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"width":"100%","margin":"16px 0 20px","flexWrap":"wrap","display":"flex"}' :inline="true" :model="searchForm">
				...
				<el-row :style='{"width":"100%","padding":"0","margin":"20px 0 20px","background":"none","display":"flex"}'>
					<el-button :style='{"border":"1px solid #c1ccf4","cursor":"pointer","padding":"0 24px","boxShadow":"0 2px 6px rgba(0,0,0,.6)","margin":"0 10px 0 0","outline":"none","color":"#c1ccf4","borderRadius":"20px","background":"rgba(243,234,250,.2)","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('discussdianyingtuijian','新增')" type="success" @click="addOrUpdateHandler()">新增</el-button>
					<el-button :style='{"border":"1px solid #d67081","cursor":"pointer","padding":"0 24px","boxShadow":"0 2px 6px rgba(0,0,0,.6)","margin":"0 10px 0 0","outline":"none","color":"#df2543","borderRadius":"20px","background":"rgba(223,37,67,.2)","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('discussdianyingtuijian','删除')" :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">删除</el-button>
			...
			<!-- <div> -->
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button :style='{"border":"1px solid #22d6c3","cursor":"pointer","padding":"0 16px","margin":"3px 6px 3px 0","outline":"none","color":"#22d6c3","borderRadius":"20px","background":"rgba(31,191,174,.1)","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('discussdianyingtuijian','修改')" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>


							<el-button :style='{"border":"1px solid #c1ccf4","cursor":"pointer","padding":"0 16px","margin":"3px 6px 3px 0","outline":"none","color":"#c1ccf4","borderRadius":"20px","background":"rgba(193,204,244,.1)","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('discussdianyingtuijian','查看评论')" type="primary" size="mini" @click="disscussListHandler(scope.row.id)">查看评论</el-button>
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      ...
      this.$http({
        url: "discussdianyingtuijian/page",
        method: "get",
        params: params
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.$router.push({path:'/discussdiscussdianyingtuijian',query:{refid:id}});
    },
    // 下载
    download(file){
      window.open(`${file}`)
    ...
    deleteHandler(id) {
      ...
      this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
        ...
        this.$http({
          url: "discussdianyingtuijian/delete",
          method: "post",
          data: ids
        }).then(({ data }) => {
          if (data && data.code === 0) {
